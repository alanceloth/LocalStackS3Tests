...
def generate_polars_data(num_clients, num_transactions, num_transaction_items):
    # num_clients = 1
    # num_transactions = 2
    ...
    transactions_df = create_transaction_data(num_transactions, clients_df["ID Cliente"].to_list())
    transaction_items_df = create_transaction_items_data(num_transaction_items, transactions_df["ID Transação"].to_list())
    
    clients_df.write_csv("app/data/polars/clientes.csv", quote_char='"')
    transactions_df.write_csv("app/data/polars/transacoes_.csv", quote_char='"')
    transaction_items_df.write_csv("app/data/polars/transacoes_itens.csv", quote_char='"')

    end_time = time.time()
    return end_time - start_time

if __name__ == "__main__":
    time_taken = generate_polars_data(1_000_000, 3_000_000, 5_000_000)
    print(f"Tempo total com Polars: {time_taken:.2f} segundos")
